...
private:
    ...
    size_t buffer_size_;
    static constexpr size_t MAX_BUFFER_SIZE = 512 * 1024;  // 512KB缓冲区（增大缓冲减少打断）
    static constexpr size_t MIN_BUFFER_SIZE = 64 * 1024;   // 64KB最小播放缓冲（增大减少卡顿）
...
public:
    ...
    // 勿扰模式 - 播放时不响应语音唤醒（防止被打断）
    std::atomic<bool> dnd_mode_;  // Do Not Disturb 模式标志
    std::atomic<bool> pending_wake_word_;  // 记录勿扰期间是否有唤醒词
    
    void EnableDndMode(bool enable) { dnd_mode_.store(enable); }
    bool IsDndModeEnabled() const { return dnd_mode_.load(); }
    bool HasPendingWakeWord() const { return pending_wake_word_.load(); }
    void ClearPendingWakeWord() { pending_wake_word_.store(false); }
    void SignalWakeWord() { pending_wake_word_.store(true); }
